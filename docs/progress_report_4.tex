...
Before implementing the designed communication protocol, we verified the correctness using Communication Sequential Processes (CSP). We modeled the robot's \texttt{GETOUT} protocol using CSP notation and checked for deadlocks using the FDR4.0~\cite{fdr}. 
\begin{align*}
  \text{\texttt{ROBOT\_MOV}} = &((\text{getout?m} \then \text{\texttt{RESPONDM}} \extchoice \text{reach} \then \text{\texttt{ROBOT\_REACH}} )    \extchoice \texttt{see} \then \texttt{SENDM}) \\ 
  \text{\texttt{ROBOT\_REACH}} = &((\text{getout?m} \then \text{\texttt{RESPONDR}} \extchoice \text{pickup} \then \text{\texttt{ROBOT\_CARRY}} ) \extchoice \texttt{see} \then \texttt{SENDR}) \\
  \text{\texttt{ROBOT\_CARRY}} = &(\text{getout?m} \then \text{\texttt{ROBOT\_CARRY}} \extchoice \text{dropbox} \then \text{\texttt{ROBOT\_MOV}} ) \\
  \text{\texttt{RESPONDM}} = &(\text{prio} \then \text{\texttt{ROBOT\_MOV}} \extchoice \text{nprio} \then \text{\texttt{ROBOT\_MOV}} ) \\
  \text{\texttt{RESPONDR}} = &(\text{prio} \then \text{\texttt{ROBOT\_MOV}} \extchoice \text{nprio} \then \text{\texttt{ROBOT\_REACH}} ) \\
  \text{\texttt{SENDM1}} = &((\text{getout!m} \then (\text{prio} \then \text{\texttt{ROBOT\_MOV}})) \extchoice \text{getout?m} \then \text{\texttt{RESPONDM}}) \\
  \text{\texttt{SENDR1}} = &((\text{getout!m} \then (\text{prio} \then \text{\texttt{ROBOT\_REACH}})) \extchoice \text{getout?m} \then \text{\texttt{RESPONDR}}) \\
  \text{\texttt{SENDM2}} = &((\text{getout!m} \then (\text{nprio} \then \text{\texttt{ROBOT\_MOV}})) \extchoice \text{getout?m} \then \text{\texttt{RESPONDM}}) \\
  \text{\texttt{SENDR2}} = &((\text{getout!m} \then (\text{nprio} \then \text{\texttt{ROBOT\_REACH}})) \extchoice \text{getout?m} \then \text{\texttt{RESPONDR}}) \\ 
  \text{\texttt{SYSTEM}} = &(\text{\texttt{ROBOT\_MOV1}} | \{ \text{getout}, \text{see}, \text{reach}, \text{prio}, \text{nprio} \text{pickup} \text{dropbox} \} | \text{\texttt{ROBOT\_MOV2}}) 
\end{align*}

To model the \texttt{GETOUT} protocol, we defined several processes to represent the robot's behavior while moving towards a target gold.

To verify the absence of deadlocks in the \texttt{GETOUT} protocol, we translated the above CSP model into $CSP_M$ syntax~\cite{cspm}, as shown in Listing~\ref{lst:csp}, and created a system of two robots moving towards a common target gold.

\begin{lstlisting}[float, caption={\texttt{GETOUT} protocol represented using $CSP_M$}, label={lst:csp}]
channel getout, see: {1, 2}
channel reach, prio, nprio, pickup, dropbox

ROBOT_MOV1 = ((getout?1 -> RESPONDM1 [] reach -> ROBOT_REACH1) 
              [] see?1 -> SENDM1)
ROBOT_REACH1 = ((getout?1 -> RESPONDR1 [] pickup -> ROBOT_CARRY1) 
                [] see?1 -> SENDR1)
ROBOT_CARRY1 = ((getout?1 -> ROBOT_CARRY1) [] dropbox -> ROBOT_MOV1)

RESPONDM1 = (prio -> ROBOT_MOV1 [] nprio -> ROBOT_MOV1)
RESPONDR1 = (prio -> ROBOT_MOV1 [] nprio -> ROBOT_REACH1)

SENDM1 = ((getout!2 -> (prio -> ROBOT_MOV1)) 
          [] getout?1 -> RESPONDM1)
SENDR1 = ((getout!2 -> (prio -> ROBOT_REACH1)) 
          [] getout?1 -> RESPONDR1)


ROBOT_MOV2 = ((getout?2 -> RESPONDM1 [] reach -> ROBOT_REACH2) 
              [] see?2 -> SENDM2)
ROBOT_REACH2 = ((getout?2 -> RESPONDR2 [] pickup -> ROBOT_CARRY2) 
                [] see?2 -> SENDR2)
ROBOT_CARRY2 = ((getout?2 -> ROBOT_CARRY2) [] dropbox -> ROBOT_MOV2)

RESPONDM2 = (prio -> ROBOT_MOV2 [] nprio -> ROBOT_MOV2)
RESPONDR2 = (prio -> ROBOT_MOV2 [] nprio -> ROBOT_REACH2)

SENDM2 = ((getout!1 -> (nprio -> ROBOT_MOV2)) 
          [] getout?2 -> RESPONDM2)
SENDR2 = ((getout!1 -> (nprio -> ROBOT_REACH2)) 
          [] getout?2 -> RESPONDR2)

SYSTEM = ROBOT_MOV1 [| 
          {| getout, see, reach, prio, nprio, pickup, dropbox |} 
        |] ROBOT_MOV2
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/fdr_result_getout.png}
    \caption{FDR4.0 Result Showing No Deadlock in the \texttt{GETOUT} Protocol}   
    \label{fig:fdr-result}  
\end{figure}

We verified the \texttt{GETOUT} protocol using the FDR4.0. From the result, shown in Figure~\ref{fig:fdr-result}, we confirmed that the system is deadlock-free.
...
After verifying the \texttt{GETOUT} protocol, we implemented the sending, as shown in Listing~\ref{lst:send-getout}, and receiving, as shown in Listing~\ref{lst:recv-getout}, part of the protocol into the robot's control code. A flowchart summarizing the overall cross-cluster communication process is presented in Figure~\ref{fig:cross-cluster-flowchart}.
...
pub fn observe(&mut self, grid: &mut Grid) {
  ...
  if  self.knowledge_base.get(&self.target_gold).has_team_robot() {
      ...
      Some(combined_pair_id) => {
        ...
        match message.message_content {
          MessageContent::Coord(Some(get_out_coord), _) => {
            if self.target_gold == get_out_coord 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{images/robot_state_diagram.png}
\caption{State Machine illustrating the robot’s internal state transitions}
