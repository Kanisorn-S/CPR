...
\begin{align}
  ...
  \text{\texttt{RESPOND}} &= (\text{ack!m} \then \text{\texttt{T}} \extchoice \text{prio} \then \text{\texttt{RESPOND}}) \label{eq:respond}\\
  \text{\texttt{WAIT}} &= (\text{ack?m} \then \texttt{T} \extchoice \text{req?m} \then \text{\texttt{RESPONDCOND}}) \label{eq:wait}\\
  \text{\texttt{RESPONDCOND}} &= ((\text{prio} \then \text{\texttt{RESPOND}} \extchoice \text{\texttt{WAIT}}) \extchoice \text{\texttt{SEND}}) \label{eq:respondcond}\\
  \text{\texttt{T}} &= (\text{turn} \then \text{\texttt{ROBOT}}) \label{eq:t}\\
  \text{\texttt{SEND1}} &= (\text{req!m} \then \text{\texttt{WAIT}} \extchoice \text{\texttt{WAIT}}) \label{eq:send1}\\
  \text{\texttt{SEND2}} &= ((\text{req!m} \then (\text{prio} \then \text{\texttt{WAIT}})) \extchoice \text{\texttt{WAIT}}) \label{eq:send2}\\
  \text{\texttt{SYSTEM}} &= (\text{\texttt{ROBOT1}} | \{ \text{req}, \text{ack}, \text{prio}, \text{turn} \} | \text{\texttt{ROBOT2}}) \label{eq:system}
\end{align}
...
\item The \texttt{RESPONDCOND} process models the robot's behavior when it has received a \\\texttt{DirectionRequest} message while waiting for an \texttt{Acknowledgment} message. In this case, if the sender has priority, it transitions into the \texttt{RESPOND} process; otherwise, it transitions back to the \texttt{WAIT} process. If the robot is not waiting for an \texttt{Acknowledgment} message (the robot has not send a \texttt{DirectionRequest}), it transitions into the \texttt{SEND} process.
...
\item The \texttt{SEND} process is divided into two cases. The first case, \texttt{SEND1}, shown in~\eqref{eq:send1}, models a robot with lower id sending a \texttt{DirectionRequest} message to a robot with higher id (it does not have priority). The second case, \texttt{SEND2}, shown in~\eqref{eq:send2}, models a robot with higher id sending a \texttt{DirectionRequest} message to a robot with lower id (it has priority). After sending the message, it transitions into the \texttt{WAIT} process to wait for an \texttt{Acknowledgment} message.
\end{itemize}
...
channel req, ack : {1, 2}
channel reach, prio, turnL, turnR

ROBOT1 = (req?1 -> RESPOND1 [] reach -> SEND)

RESPOND1 = (ack!2 -> T1R [] prio -> RESPOND1)
SEND = (req!2 -> WAIT1 [] WAIT1)
WAIT1 = (ack?1 -> T1L [] req?1 -> RESPONDCOND1)
RESPONDCOND1 = ((prio -> RESPOND1 [] WAIT1) [] SEND)
T1L = turnL -> ROBOT1
T1R = turnR -> ROBOT1

ROBOT2 = (req?2 -> RESPOND2 [] reach -> SENDP)

RESPOND2 = (ack!1 -> T2L [] prio -> RESPOND2)
SENDP = ((req!1 -> (prio -> WAIT2)) [] WAIT2)
WAIT2 = (ack?2 -> T2R [] req?2 -> RESPONDCOND2)
RESPONDCOND2 = ((prio -> RESPOND2 [] WAIT2) [] SENDP)
T2L = turnL -> ROBOT2
T2R = turnR -> ROBOT2


SYSTEM = ROBOT1 [| {| req, ack, prio, turnL, turnR |} |] ROBOT2
